FROM docker.io/alpine:3.22

# Install required dependencies
RUN apk add php84 \
            php84-ctype \
            php84-curl \
            php84-dom \
            php84-gd \
            php84-iconv \
            libxml2 \
            php84-xml \
            php84-mbstring \
            php84-openssl \
            php84-posix \
            php84-session \
            php84-simplexml \
            php84-xmlreader \
            php84-xmlwriter \
            php84-zip \
            php84-pdo \
            php84-pdo_pgsql \
            php84-fileinfo \
            php84-bz2 \
            php84-intl \
            php84-ldap \
            php84-pecl-apcu \
            ffmpeg \
            php84-pecl-imagick \
            php84-fpm \
            nginx \
            supervisor \
            gpg \
            gpg-agent

# Download the latest Nextcloud tarball
WORKDIR /var/www

# Download the latest tarball with checksums and signatures
RUN wget https://download.nextcloud.com/server/releases/latest.tar.bz2
RUN wget https://download.nextcloud.com/server/releases/latest.tar.bz2.sha256
RUN wget https://download.nextcloud.com/server/releases/latest.tar.bz2.asc

# Validate the checksum
RUN grep latest.tar.bz2 latest.tar.bz2.sha256 | sha256sum -c

# Import nextcloud PGP key
RUN cat <<EOF | gpg --import
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v2


M704NM/xbc23
=ykUA
-----END PGP PUBLIC KEY BLOCK-----
EOF

# Verify the signature of tarball
RUN gpg --verify latest.tar.bz2.asc

# Extract the archive
RUN tar -xvf latest.tar.bz2
RUN chown -R nginx:nginx nextcloud

# Cleanup
RUN rm latest.tar.bz2 latest.tar.bz2.asc latest.tar.bz2.sha256

##########################
### Configuration time ###
##########################

COPY nginx.conf /etc/nginx/http.d/nextcloud.conf
RUN rm /etc/nginx/http.d/default.conf
COPY supervisord.conf /etc/supervisord.conf
COPY fpm.conf /etc/fpm.conf
RUN mkdir -p /var/run/php
RUN chown -R nginx:nginx /var/run/php

CMD [ "/usr/bin/supervisord", "-c", "/etc/supervisord.conf" ]